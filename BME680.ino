...
void setup() {
  Serial.begin(115200);
  while (!Serial);
  Serial.println(F("{\"status\":\"BME680 async test\"}"));

  if (!bme.begin()) {
    Serial.println(F("{\"status\":\"Could not find a valid BME680 sensor, check wiring!\"}"));
    while (1);
  }
...
void loop() {
  ...
  if (endTime == 0) {
    Serial.println(F("{\"status\":\"Failed to begin reading :(\"}"));
    return;
  }
  ...
  */
  ...
  unsigned long completed = millis();
  ...
  Serial.print(F(",\"gas_kohms\":"));
  Serial.print(bme.gas_resistance / 1000.0);
  //Serial.println(F(" KOhms"));

  Serial.print(F(",\"approxalt_m\":"));
